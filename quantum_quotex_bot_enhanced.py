...
def normalize_candles(raw: list) -> List[Dict]:
    """
    Normalize any candle payload into a clean list of OHLC dicts.

    Quotex sometimes returns full OHLC dicts and sometimes only price points.
    The old code turned price points into candles where open==high==low==close,
    which made EVERY strategy and the backtester see a flat market (so every
    trade scored as a loss). Here we rebuild proper OHLC: the previous close
    becomes the current open and a small synthetic wick is derived from the
    move so the candles actually have a body and direction.
    """
    out: List[Dict] = []
    prev_close = None
    ...
    for c in raw:
        if isinstance(c, dict):
            close = float(c.get('close', c.get('open', prev_close or 0)) or 0)
            open_ = float(c.get('open', prev_close if prev_close is not None else close))
            high = float(c.get('high', max(open_, close)))
            low = float(c.get('low', min(open_, close)))
            # Guard against degenerate (flat) candles from the feed
            if high <= low:
                spread = abs(close - open_) or max(abs(close) * 1e-5, 1e-6)
            ...
            move = close - open_
            wick = abs(move) * 0.4 if move else max(abs(close) * 1e-5, 1e-6)
            candle = {
                'time': time.time(),
                'open': open_,
...
class AdvancedIndicators:
    ...
    def _ema_series(data: List[float], period: int) -> List[float]:
        """Return the full EMA series for `data` (one value per input point)."""
        if not data:
            return []
    ...
    def macd_oscillator(closes: List[float], fast: int = 12, 
        ...
        """Proper MACD: real EMA series, signal line and histogram with crossover history."""
        if len(closes) < slow + signal:
            return {}

        ema_fast = AdvancedIndicators._ema_series(closes, fast)
        ema_slow = AdvancedIndicators._ema_series(closes, slow)

        # MACD line = fast EMA - slow EMA, computed for every bar
        macd_series = [f - s for f, s in zip(ema_fast, ema_slow)]

        # Signal line = EMA of the MACD line
        signal_series = AdvancedIndicators._ema_series(macd_series, signal)

        macd_line = macd_series[-1]
        signal_line = signal_series[-1]
        histogram = macd_line - signal_line

        prev_hist = (macd_series[-2] - signal_series[-2]) if len(macd_series) >= 2 else histogram

        # Real divergence: compare price swing vs MACD swing over the last 10 bars
        divergence = None
        if len(macd_series) >= 10:
            price_now, price_then = closes[-1], closes[-10]
            macd_now, macd_then = macd_series[-1], macd_series[-10]
    ...
    def rsi_pattern_recognition(closes: List[float], period: int = 14) -> Dict[str, Any]:
        ...
        # Real divergence: compute RSI now vs RSI `period` bars ago and compare to price
        if len(closes) >= period * 2 + 1:
            prev_gain = sum(gains[-2 * period:-period]) / period
            prev_loss = sum(losses[-2 * period:-period]) / period
            if prev_loss == 0:
                prev_rsi = 100
            else:
                prev_rsi = 100 - (100 / (1 + prev_gain / prev_loss))

            if closes[-1] > closes[-period] and rsi < prev_rsi:
                patterns.append("BEARISH_DIVERGENCE")
            elif closes[-1] < closes[-period] and rsi > prev_rsi:
...
class QuantumPredictionEngine:
    ...
    def predict(self, candles: List[Dict]) -> Optional[PredictionResult]:
        """
        Consensus prediction across ALL active strategies.

        Instead of trusting one strategy's hard-coded confidence number, we run
        every active strategy, collect the ones that actually fire, and let them
        vote. The final confidence reflects how strongly the strategies agree,
        which is a far better real-world quality filter than a single constant.
        """
        signals: List[PredictionResult] = []
        ...
        # If only one strategy is active, return its signal directly
        if len(self.active_strategies) == 1:
            self.total_predictions += 1
            return signals[0]

        # Weighted vote: each strategy contributes its confidence to its side
        ...
        call_count = sum(1 for s in signals if s.direction == "CALL")
        put_count = sum(1 for s in signals if s.direction == "PUT")
        ...
        if call_weight >= put_weight:
            direction = "CALL"
            agreement = call_weight / total_weight
            agreeing = [s for s in signals if s.direction == "CALL"]
            votes = call_count
        else:
            direction = "PUT"
            agreement = put_weight / total_weight
            agreeing = [s for s in signals if s.direction == "PUT"]
            votes = put_count

        # Confidence = how strongly strategies agree, scaled to a 50-98 range.
        confidence = round(min(98.0, 50 + agreement * 48), 1)

        avg_move = sum(abs(s.next_candle.get('move', 0)) for s in agreeing) / max(len(agreeing), 1)
        current_price = candles[-1]['close']
        predicted_close = current_price + avg_move if direction == "CALL" else current_price - avg_move
        ...
        return PredictionResult(
            ...
            indicators={
                'agreeing_strategies': votes,
                'total_firing': len(signals),
                'agreement_pct': round(agreement * 100, 1),
            },
            bull_score=round(call_weight / total_weight * 100, 1),
            bear_score=round(put_weight / total_weight * 100, 1),
            strategy_name=f"Consensus ({votes}/{len(signals)} agree)",
        )
...
class QuantumTradingBotEnhanced:
    ...
    def __init__(self):
        ...
        self.trade_durations = [60]      # configurable expiries (#11)
        self.trade_interval = 60         # seconds between trades (#6)
        self.shuffle_assets = True       # shuffle & trade all assets (#12)

        # Markets (full duration set available per asset)
        durations = [5, 10, 15, 30, 60, 120, 180, 300, 600]
        self.markets = {
            asset: list(durations)
            for asset in [
                "EURUSD_otc", "GBPUSD_otc", "USDJPY_otc", "AUDCAD_otc",
                "EURGBP_otc", "AUDUSD_otc", "EURJPY_otc", "GBPJPY_otc",
                "USDCAD_otc", "NZDUSD_otc", "EURAUD_otc", "GBPCAD_otc",
            ]
        }
    ...
    async def _set_trading_parameters(self):
        ...
        self.min_confidence = float(input("🎯 Min Confidence % (60-98): ") or "75")
        ...
        # ---- Trade durations (#11): user can enable 5/10/30/60 ... up to 600s ----
        console.print("\n[bold]⏱️ Trade Durations (expiry in seconds):[/bold]")
        console.print("[dim]Allowed: 5, 10, 15, 30, 60, 120, 180, 300, 600[/dim]")
        console.print("[dim]Enter values separated by commas (e.g. 5,30,60,300) or 'all'.[/dim]")
        allowed = [5, 10, 15, 30, 60, 120, 180, 300, 600]
        dur_in = input("👉 Durations: ").strip().lower()
        if dur_in == "all":
            self.trade_durations = allowed
        elif dur_in:
            picked = []
            for token in dur_in.split(","):
                token = token.strip()
                if token.isdigit() and int(token) in allowed:
                    picked.append(int(token))
            self.trade_durations = picked or [60]
        else:
            self.trade_durations = [60]
        console.print(f"[green]✅ Durations: {', '.join(str(d) + 's' for d in self.trade_durations)}[/green]")

        # ---- How often to take a trade (#6): default 1 minute ----
        interval_in = input("\n🕐 Seconds between trades (default 60): ").strip()
        self.trade_interval = int(interval_in) if interval_in.isdigit() else 60

        # ---- Trade across all assets / shuffle them (#12) ----
        shuffle_in = input("🔀 Shuffle & trade ALL assets each round? (y/n, default y): ").strip().lower()
        self.shuffle_assets = shuffle_in != "n"
    ...
    async def scan_all_markets(self) -> List[Dict]:
        """
        Scan every asset across every configured duration, in parallel, and
        return the qualifying opportunities sorted best-first.

        - Assets are shuffled each round so trading is spread across all of
          them instead of always favouring the same pair (#12).
        - All durations the user enabled are evaluated (#11).
        - Candle fetches run concurrently so a full scan finishes in ~seconds
          instead of stalling on each request one by one (#4).
        """
        import random

        assets = list(self.markets.keys())
        if self.shuffle_assets:
            random.shuffle(assets)

        # Build the (asset, period) work list
        tasks = []
        meta = []
        for asset in assets:
            for period in self.trade_durations:
                meta.append((asset, period))
                tasks.append(self._get_candles(asset, period, 120))

        candle_sets = await asyncio.gather(*tasks, return_exceptions=True)

        opportunities = []
        for (asset, period), candles in zip(meta, candle_sets):
    ...
    async def _get_candles(self, asset: str, period: int, count: int) -> List[Dict]:
        ...
        try:
            ...
            if candles:
                return normalize_candles(candles[-count:])
        except Exception as e:
    ...
    async def run(self):
        ...
        try:
            while self.is_running:
                ...
                # Scan for opportunities (parallel, shuffled across all assets)
                opportunities = await self.scan_all_markets()
                
                if opportunities:
                    # Best, highest-agreement opportunity this round
                    ...
                    # Short execute countdown (no wasted analysis time)
                    console.print(f"\n[bold red]🚨 EXECUTING NOW! 🚨[/bold red]")
                    ...
                    # ---- Running P/L after EVERY trade (#8) ----
                    self._show_running_pl(trade_count)

                    # Pace trades by the configured interval (#6)
                    await asyncio.sleep(max(0, self.trade_interval - best['period']))
                
                else:
                    console.print(
                        f"\r[dim]⏳ Scanning all assets... ({datetime.now().strftime('%H:%M:%S')})[/dim]",
                        end="",
                    )
...
# ==================== NEW RSI HEAD & SHOULDERS STRATEGY ====================
class RSIHeadShouldersStrategy:
    """RSI Head & Shoulders Pattern Recognition Strategy"""
    ...
    @staticmethod
    def calculate_rsi(closes: List[float], n: int = 14) -> List[float]:
        ...
        # Calculate RS using SMMA
        smma_up = RSIHeadShouldersStrategy.calculate_smma(up, n)
        smma_down = RSIHeadShouldersStrategy.calculate_smma(down, n)
        
        rs = []
        for u, d in zip(smma_up, smma_down):
            if d == 0:
                rs.append(100.0)
            else:
                rs.append(u / d)
        
        output = [100 - 100 / (1 + r) for r in rs]
        return [closes[0]] * n + output
    
    @staticmethod
    def detect_head_shoulders_pattern(rsi_values: List[float], 
                                      closes: List[float],
                                      period: int = 25,
                                      delta: float = 0.2,
                                      head_multiplier: float = 1.1,
                                      shoulder_multiplier: float = 1.1) -> Optional[Dict[str, Any]]:
        """
        Detect Head & Shoulders pattern on RSI
        ...
        for i in range(period, len(rsi_values)):
            ...
            # Check if no holding positions and close price is not the maximum
            if (rsi_values[i] != max(rsi_values[i-period:i])):
                
                # Find head node j with maximum RSI value in pattern finding period
                j = i - period + rsi_values[i-period:i].index(max(rsi_values[i-period:i]))
                
                # If head node j is significantly larger than node i
                if (abs(rsi_values[j] - rsi_values[i]) > head_multiplier * delta):
                    bottom = rsi_values[i]
                    moveon = True
                
                # Find node k between node j and node i
                if moveon:
                    moveon = False
                    for k in range(j, i):
                        if (abs(rsi_values[k] - bottom) < delta):
                            moveon = True
                            break
                
                # Find node l between node j and the end of pattern finding horizon
                if moveon:
                    moveon = False
                    for l_ in range(j, i - period + 1, -1):
                        if (abs(rsi_values[l_] - bottom) < delta):
                            moveon = True
                            l = l_
                            break
                
                # Find node m between node l and the end of pattern finding horizon
                if moveon:
                    moveon = False
                    for m in range(i - period, l if 'l' in locals() else i):
                        if (abs(rsi_values[m] - bottom) < delta):
                            moveon = True
                            break
                
                # Get shoulder node n with maximum value between node m and node l
                if moveon and 'l' in locals() and 'm' in locals():
                    moveon = False
                    n = m + rsi_values[m:l].index(max(rsi_values[m:l])) if l > m else m
                    if (rsi_values[n] - bottom > shoulder_multiplier * delta) and \
                       (rsi_values[j] - rsi_values[n] > shoulder_multiplier * delta):
                        top = rsi_values[n]
                        moveon = True
                
                # Find shoulder node o between node k and node i
                if moveon and 'k' in locals():
                    for o in range(k, i):
                        if (abs(rsi_values[o] - top) < delta):
                            return {
                                'pattern_detected': True,
                                'pattern_type': 'HEAD_SHOULDERS',
                                'signal': 'SHORT',  # Bearish signal
                                'coordinates': {
                                    'left_shoulder_start': m if 'm' in locals() else i,
                                    'left_shoulder': n if 'n' in locals() else i,
                                    'head': j,
                                    'neckline_left': l if 'l' in locals() else i,
                                    'neckline_right': k if 'k' in locals() else i,
                                    'right_shoulder': o,
                                    'completion': i
                                }
                            }
...
# Add the RSI Head & Shoulders strategy to QuantumPredictionEngine
def _strategy_rsi_head_shoulders(self, candles: List[Dict]) -> Optional[PredictionResult]:
    """RSI Head & Shoulders Pattern Recognition Strategy"""
    if len(candles) < 50:
        return None
    ...
    # Calculate RSI using SMMA method
    rsi_values = RSIHeadShouldersStrategy.calculate_rsi(closes, n=14)
    
    if not rsi_values or len(rsi_values) < 30:
        return None
    
    # Check for overbought/oversold signals
    current_rsi = rsi_values[-1]
    
    # Overbought/Oversold strategy (basic)
    if current_rsi > 70:
        direction = "PUT"  # Overbought - expect price to fall
        confidence = 70
        signal_type = "OVERBOUGHT"
    elif current_rsi < 30:
        direction = "CALL"  # Oversold - expect price to rise
        confidence = 70
        signal_type = "OVERSOLD"
    else:
        # Try Head & Shoulders pattern detection
        pattern_result = RSIHeadShouldersStrategy.detect_head_shoulders_pattern(
            rsi_values, closes, period=25
        )
        
        if pattern_result and pattern_result['pattern_detected']:
            direction = "PUT"  # Head & Shoulders is bearish
            confidence = 75
            signal_type = "HEAD_SHOULDERS"
        else:
            return None
    
    # Calculate predicted move based on ATR
    atr = sum(abs(closes[i] - closes[i-1]) for i in range(-14, 0)) / 14
    predicted_move = atr * 0.8
    ...
    # Enhanced confidence for Head & Shoulders pattern
    if signal_type == "HEAD_SHOULDERS":
        confidence = min(85, confidence + 10)
    elif signal_type in ["OVERBOUGHT", "OVERSOLD"]:
        # Confidence increases as RSI moves further from center
        rsi_extremeness = abs(current_rsi - 50) / 20
        confidence = min(85, 65 + rsi_extremeness * 10)
    ...
    return PredictionResult(
        ...
        indicators={
            ...
            'rsi_pattern': signal_type
        },
        bull_score=80 if direction == "CALL" else 20,
        bear_score=20 if direction == "CALL" else 80,
        strategy_name="RSI Head & Shoulders"
    )

# Monkey-patch the strategy into QuantumPredictionEngine
QuantumPredictionEngine._strategy_rsi_head_shoulders = _strategy_rsi_head_shoulders

# Add to strategies dictionary (modify the __init__ method)
original_init = QuantumPredictionEngine.__init__

def new_init(self):
    original_init(self)
    self.strategies['rsi_head_shoulders'] = self._strategy_rsi_head_shoulders

QuantumPredictionEngine.__init__ = new_init

# Add to available strategies list in EnhancedBacktestEngine
original_backtest_init = EnhancedBacktestEngine.__init__

def new_backtest_init(self, client):
    original_backtest_init(self, client)
    if 'rsi_head_shoulders' not in self.available_strategies:
        self.available_strategies.append('rsi_head_shoulders')

EnhancedBacktestEngine.__init__ = new_backtest_init

# Add to strategy selection menu in QuantumTradingBotEnhanced._select_strategies
original_select_strategies = QuantumTradingBotEnhanced._select_strategies

async def new_select_strategies(self):
    # Modify the strategies list to include the new strategy
    await original_select_strategies(self)

# Add the new strategy to the menu display
QuantumTradingBotEnhanced._select_strategies = new_select_strategies

# Add the strategy number to the menu
# This will be handled by modifying the strategies_list in _select_strategies method
original_menu = QuantumTradingBotEnhanced._select_strategies

async def enhanced_select_strategies(self):
    """Enhanced strategy selection with RSI Head & Shoulders"""
    console.print("\n[bold magenta]🎯 STRATEGY SELECTION[/bold magenta]")
    console.print("\n[dim]Available Strategies:[/dim]")
    ...
    strategies_list = [
        ...
        ("11", "options_straddle", "Options Straddle"),
        ("12", "pair_trading", "Pair Trading"),
        ("13", "rsi_head_shoulders", "RSI Head & Shoulders Pattern")  # NEW
    ]
    ...
    console.print("\n[bold yellow]Select strategies:[/bold yellow]")
    console.print("  • Enter numbers separated by commas (e.g., 1,3,7,13)")
    console.print("  • Enter 'all' for all strategies")
    console.print("  • Enter '0' for default (Quantum Composite)")
...
# Replace the method
QuantumTradingBotEnhanced._select_strategies = enhanced_select_strategies

console.print("\n[bold green]✅ RSI Head & Shoulders Strategy Added Successfully![/bold green]")
console.print("[dim]Strategy #13 now available in strategy selection menu[/dim]")
